...
def mapMatplotlib(merge_df):
    ...
    merge_df[merge_df['month'] == 2].plot(ax=ax[0],column='mean',cmap='Pastel1',
                                          legend=False, alpha=0.9, edgecolor='gray')
    merge_df[merge_df['month'] == 3].plot(ax=ax[1],column='mean',cmap='Pastel1',
                                          legend=False, alpha=0.9, edgecolor='gray')
...
def showMap(total_df):
    ...
    total_df = total_df[['DEAL_YMD','month','SGG_CD','OBJ_AMT','HOUSE_TYPE']].reset_index(drop=True)
